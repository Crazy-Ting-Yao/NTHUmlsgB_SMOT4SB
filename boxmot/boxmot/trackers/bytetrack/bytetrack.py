...
from boxmot.utils.matching import fuse_score, iou_distance, linear_assignment, giou_distance
...
class ByteTrack(BaseTracker):
    ...
    def update(self, dets: np.ndarray, img: np.ndarray = None, embs: np.ndarray = None) -> np.ndarray:
        ...
        dists = giou_distance(strack_pool, detections)
        ...
        dists = giou_distance(r_tracked_stracks, detections_second)
        ...
        dists = giou_distance(unconfirmed, detections)
        # if not self.args.mot20:
        dists = fuse_score(dists, detections)
...
def remove_duplicate_stracks(stracksa, stracksb):
    pdist = giou_distance(stracksa, stracksb)

...
class AssociationFunction:
    ...
    @staticmethod
    def so_hota_batch(bboxes1, bboxes2, iou_thr=0.5) -> np.ndarray:
        """
        Compute SO-HOTA (Self-Optimal Higher Order Tracking Accuracy) association scores.
        
        Parameters:
        - bboxes1: (N, 4) array of predicted bounding boxes [x1, y1, x2, y2]
        - bboxes2: (M, 4) array of ground truth bounding boxes [x1, y1, x2, y2]
        - iou_thr: IoU threshold for valid matches (default is 0.5)
        
        Returns:
        - cost_matrix: (N, M) matrix where (i, j) contains the SO-HOTA association score
        """
        # Step 1: Compute IoU matrix
        iou_matrix = AssociationFunction.iou_batch(bboxes1, bboxes2)  # shape (N, M)

        # Step 2: Filter by IoU threshold (we now allow a smaller margin for nearby detections)
        match_matrix = iou_matrix >= iou_thr  # matches above threshold
        
        # Step 3: Build match cost matrix (1 if matched, 0 otherwise)
        cost_matrix = np.zeros_like(iou_matrix)
        cost_matrix[match_matrix] = iou_matrix[match_matrix]
        
        # Step 4: Normalize by row/column sums (geometric mean version of SO-HOTA)
        row_sums = (match_matrix.sum(axis=1) + 1e-6)  # for predictions
        col_sums = (match_matrix.sum(axis=0) + 1e-6)  # for ground truths
        normalizer = np.sqrt(np.outer(row_sums, col_sums))  # shape (N, M)
        
        # Normalize the cost matrix to get similarity: higher values = better match
        cost_matrix = cost_matrix / normalizer
        
        # Step 5: Return final cost matrix (1 - cost_matrix to make it a distance metric)
        return cost_matrix

...
def iou_distance(atracks, btracks):
    """
    Compute cost based on IoU or Kalman-predicted boxes
    :type atracks: list[STrack]
    :type btracks: list[STrack]
    :rtype cost_matrix: np.ndarray
    """
    ...
    if isinstance(atracks[0], np.ndarray) or isinstance(btracks[0], np.ndarray):
        ...
        try:
            # 嘗試用 track.xyxy，如果沒有再 fallback 用 Kalman 預測框
            atlbrs = [track.xyxy for track in atracks]
            btlbrs = [track.xyxy for track in btracks]
        except AttributeError:
            atlbrs = [tlwh_to_tlbr(track.mean[:4]) for track in atracks]
            btlbrs = [tlwh_to_tlbr(track.mean[:4]) for track in btracks]
...
def giou_distance(atracks, btracks):
    """
    Compute cost based on GIoU or Kalman-predicted boxes
    :type atracks: list[STrack]
    :type btracks: list[STrack]
    :rtype cost_matrix: np.ndarray
    """
    ...
    _gious = AssociationFunction.giou_batch(atlbrs, btlbrs)
